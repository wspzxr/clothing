...
const FoodModel= require('../db/model/orderModel')
async function  add(title,permission,arttype,settop){
  // async 函数内部只要不出错 肯定走的是then 如果出错走的是catch
   let result =await FoodModel.insertMany({title,permission,arttype,settop})
   console.log(result)
}
async function get(page,pageSize){
  ...
  let books = await FoodModel.find().skip((page-1)*pageSize).limit(pageSize)
...
// 修改
async function  update(foodId,title,permission,arttype,settop){
  
  let result  = await FoodModel.updateOne({_id:foodId},{title,permission,arttype,settop})
   console.log(result)
   return  result
}
